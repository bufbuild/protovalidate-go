...
type StringInOneof struct {
	state          protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Foo isStringInOneof_Foo    `protobuf_oneof:"foo"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}
...
func (x *StringInOneof) GetBar() string {
	if x != nil {
		if x, ok := x.xxx_hidden_Foo.(*stringInOneof_Bar); ok {
			return x.Bar
		}
	}
	return ""
...
func (x *StringInOneof) SetBar(v string) {
	x.xxx_hidden_Foo = &stringInOneof_Bar{v}
}
...
func (x *StringInOneof) HasBar() bool {
	...
	_, ok := x.xxx_hidden_Foo.(*stringInOneof_Bar)
	return ok
}
...
func (x *StringInOneof) ClearBar() {
	if _, ok := x.xxx_hidden_Foo.(*stringInOneof_Bar); ok {
		x.xxx_hidden_Foo = nil
	}
}
...
func (b0 StringInOneof_builder) Build() *StringInOneof {
	...
	_, _ = b, x
	if b.Bar != nil {
		x.xxx_hidden_Foo = &stringInOneof_Bar{*b.Bar}
	}
	return m0
}
